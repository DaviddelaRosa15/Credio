    ...
    public sealed class GetEmployeeByIdQueryHandler : IQueryHandler<GetEmployeeByIdQuery, EmployeeDetailDTO>
    {
        ...
        public async Task<Result<EmployeeDetailDTO>> Handle(GetEmployeeByIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var foundEmployee = await _employeeRepository.GetByIdWithIncludeAsync(x => x.Id == request.Id,
                [
                    x => x.Address,
                    x => x.Clients
                ]);
                ...
                if (foundEmployee is null)
                    return Result<EmployeeDetailDTO>.Failure(Error.NotFound("No se encontro el empleado con el id dado"));
            ...
            {
                return Result<EmployeeDetailDTO>.Failure(
                    Error.InternalServerError($"Ocurrio un error inesperado consultando el empleado")
                );
            }
        }
    }
